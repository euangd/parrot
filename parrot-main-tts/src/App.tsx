...
function App() {
  ...
  useEffect(() => {
    const unlistenPromise = Promise.all([
      ...
      listen("tts-no-selection", () => {
        toast.error("Select text first, then press the shortcut.");
      }),
    ]);

    return () => {
      void unlistenPromise.then((unsubscribers) => {
  ...
  useEffect(() => {
    if (onboardingStep === "done" && !hasCompletedPostOnboardingInit.current) {
      ...
      // Proactively load the TTS model in the background so it's ready
      // before the user's first TTS request (avoids ~10s on-demand load delay).
      commands.preloadTtsModel().catch((e) => {
