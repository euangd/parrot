...
/// Register the play/pause shortcut (called when TTS speaking starts)
...
/// Unregister the play/pause shortcut (called when TTS speaking ends)
...
pub fn change_tts_speed_setting(app: AppHandle, speed: f32) -> Result<(), String> {
    let mut s = settings::get_settings(&app);
    s.tts_speed = speed.clamp(0.5, 2.0);
    settings::write_settings(&app, s);
    Ok(())
}
...
pub fn change_tts_shorten_first_chunk_setting(app: AppHandle, enabled: bool) -> Result<(), String> {
    let mut s = settings::get_settings(&app);
    s.tts_shorten_first_chunk = enabled;
    settings::write_settings(&app, s);
    Ok(())
}
