...
///
/// Returns `(Option<PathBuf>, Option<PathBuf>)` for the binary and data dir.
/// Passed to `KokoroModelParams` so tts-rs can locate espeak-ng without
/// relying on environment variables or PATH.
///
/// Best-effort: returns `None` if bundled files are missing (e.g. dev build
/// without resources) and tts-rs falls back to system-installed espeak-ng.
...
fn initialize_core_logic(
    ...
    // Create the speaking (TTS) overlay window (hidden by default)
    overlay::create_speaking_overlay(app_handle);
}
...
pub fn run(cli_args: CliArgs) {
    ...
    let specta_builder = Builder::<tauri::Wry>::new()
        .commands(collect_commands![
            ...
            commands::get_tts_model_status,
            commands::preload_tts_model,
            commands::toggle_tts_pause,
            commands::resize_tts_overlay,
