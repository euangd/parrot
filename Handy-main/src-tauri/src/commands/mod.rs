...
#[tauri::command]
#[specta::specta]
pub fn get_tts_model_status(tts_manager: State<'_, Arc<TTSManager>>) -> Result<TtsModelStatus, String> {
    Ok(TtsModelStatus {
        is_loaded: tts_manager.is_model_loaded(),
        is_loading: tts_manager.is_model_loading(),
    })
}
...
pub fn toggle_tts_pause(tts_manager: State<'_, Arc<TTSManager>>) -> Result<bool, String> {
    tts_manager.toggle_pause().map_err(|e| e.to_string())
}

#[tauri::command]
#[specta::specta]
pub fn resize_tts_overlay(app: AppHandle, content_height: f64) {
    crate::overlay::resize_overlay_to_content(&app, content_height);
}

#[tauri::command]
#[specta::specta]
pub fn get_tts_voices(tts_manager: State<'_, Arc<TTSManager>>) -> Result<Vec<String>, String> {
    tts_manager.get_available_voices().map_err(|e| e.to_string())
}
