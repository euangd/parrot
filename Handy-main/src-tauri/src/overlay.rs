...
// ============================================================================
// Speaking overlay (TTS)
// ============================================================================

#[derive(Serialize, Clone)]
struct SpeakingOverlayPayload {
    state: String,
    text: Option<String>,
}

const SPEAKING_OVERLAY_WIDTH: f64 = 572.0;
const SPEAKING_PROCESSING_HEIGHT: f64 = 46.0;
const SPEAKING_HEIGHT: f64 = 114.0;

fn speaking_height_for_state(state: &str) -> f64 {
    match state {
        "speaking" => SPEAKING_HEIGHT,
        _ => SPEAKING_PROCESSING_HEIGHT,
    }
}

fn calculate_speaking_overlay_position(
    ...
    let x = monitor_x + (monitor_width - SPEAKING_OVERLAY_WIDTH) / 2.0;
...
fn resize_speaking_overlay(
    ...
    if let Some((x, y)) = calculate_speaking_overlay_position(app_handle, height) {
        let _ = overlay_window
            .set_position(tauri::Position::Logical(tauri::LogicalPosition { x, y }));
    }
}
...
pub fn create_speaking_overlay(app_handle: &AppHandle) {
    let position =
        calculate_speaking_overlay_position(app_handle, SPEAKING_PROCESSING_HEIGHT);

    #[cfg(not(target_os = "linux"))]
    if position.is_none() {
        log::debug!("Failed to determine speaking overlay position, not creating overlay window");
        return;
    }
    ...
    .resizable(false)
    .focusable(false)
    .inner_size(SPEAKING_OVERLAY_WIDTH, SPEAKING_PROCESSING_HEIGHT)
    .shadow(false)
    .maximizable(false)
    .minimizable(false)
    .closable(false)
    ...
    match builder.build() {
        Ok(window) => {
            ...
            {
                if init_gtk_layer_shell(&window) {
                    log::debug!("GTK layer shell initialized for speaking overlay window");
                } else {
                    log::debug!(
                        "GTK layer shell not available for speaking overlay, using regular window"
                    );
                }
            }
            log::debug!("Speaking overlay window created successfully (hidden)");
        }
        Err(e) => {
            log::debug!("Failed to create speaking overlay window: {}", e);
        }
    }
}
...
pub fn create_speaking_overlay(app_handle: &AppHandle) {
        ...
        calculate_speaking_overlay_position(app_handle, SPEAKING_PROCESSING_HEIGHT)
    {
        match PanelBuilder::<_, SpeakingOverlayPanel>::new(app_handle, "speaking_overlay")
            ...
            .size(tauri::Size::Logical(tauri::LogicalSize {
                width: SPEAKING_OVERLAY_WIDTH,
                height: SPEAKING_PROCESSING_HEIGHT,
            }))
            .has_shadow(false)
            .transparent(true)
            .no_activate(true)
...
fn show_speaking_overlay_state(app_handle: &AppHandle, state: &str, text: Option<String>) {
    let settings = settings::get_settings(app_handle);
    if settings.overlay_position == OverlayPosition::None {
        return;
    }
    ...
    if let Some(overlay_window) = app_handle.get_webview_window("speaking_overlay") {
        let height = speaking_height_for_state(state);
        resize_speaking_overlay(app_handle, &overlay_window, height);
        ...
        let payload = SpeakingOverlayPayload {
...
pub fn show_speaking_overlay(app_handle: &AppHandle, text: Option<String>) {
    show_speaking_overlay_state(app_handle, "speaking", text);
}

/// Shows the TTS processing overlay window (before audio is ready).
pub fn show_tts_processing_overlay(app_handle: &AppHandle) {
    show_speaking_overlay_state(app_handle, "processing", None);
}

/// Resizes the speaking overlay window to fit content height from the frontend.
pub fn resize_overlay_to_content(app_handle: &AppHandle, content_height: f64) {
    let window_height = content_height + 10.0;
    if let Some(overlay_window) = app_handle.get_webview_window("speaking_overlay") {
        resize_speaking_overlay(app_handle, &overlay_window, window_height);
    }
}
